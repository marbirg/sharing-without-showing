...
svm_model *svm_load_model(const char *model_file_name)
{
  ...
  printf("Number of elements to allocate:%d\n", elements);
  printf("l=%d\n", l);
  int j=0;
  for(i=0;i<l;i++)
    {
      ...
      // printf("Adding sv_node number %d to model as number %d\n", j, i);
      p = strtok(line, " \t");
      ...
      while(1)
	{
	  ...
	  // printf("x_space[j=%d]: index=%d, value=%f\n", j, x_space[j].index, x_space[j].value);
...
int pop_str(const char* string, char c, int buffer_size, char *res){
  for (int i=0; i<buffer_size; i++){
    if (string[i]==c || string[i]=='\n' || string[i]=='\0'){
      res[i]='\0';
      // if (string[i]=='\0'){
      // 	printf("Reached end of string i=%d\n", i);
      // }
      return i;
      //   string+=(i+1);
      //   break;
...
bool read_model_header_from_string(const char *model_string, svm_model* model)
{
  svm_parameter& param = model->param;
  ...
  while(1)
    {
      l = pop_str(model_string, ' ', max_len, cmd);
      model_string+=(l+1);

      if(strcmp(cmd,"svm_type")==0)
	{
	  l = pop_str(model_string, '\n', max_len, cmd);
	  model_string+=(l+1);
...
int count_elements(const char *model_string){
  int elements = 0;
  int l;
  char * p;
  while(1)
...
svm_model *svm_load_model_from_string(const char *model_string)
{
  ...
  // read header
  if (!read_model_header_from_string(model_string, model))
    {
      fprintf(stderr, "ERROR: fscanf failed to read model\n");
      setlocale(LC_ALL, old_locale);
      free(old_locale);
  ...
  int n=1;
  while(n>0){
    n = pop_str(model_string, '\n', 1024, cmd);
    model_string+=(n+1);
    // printf("cmd=%s\n", cmd);
    if (strcmp(cmd, "SV")==0){
      // printf("Done, headers removed\n");
      break;
    }
  }
  ...
  int elements = count_elements(model_string);
  elements += model->l;
