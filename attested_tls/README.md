## Building
1. Source openenclaverc from <install_path>/shared/openenclave/openenclaverc
Ex: `source  /opt/openenclave/share/openenclave/openenclaverc`

### Build GSL
Clone latest version of the repository:
git clone https://github.com/ampl/gsl

#### Build for OpenEnclave
Since OpenEnclave implements their own functionality for some libraries (for example an own version of printf to be able to print from the enclave via the host), the GSL library needs to be build using these libraries. This can be done by specifying the path to the repository and running
`make build-gsl`
This should only need to be done once.
These libraries and their header files then needs to be linked when building the mbedtls_server. Check the Makefile in server/enclave to see how to specify the relevant paths.

#### Build for non-envlave
Follow build instructions in repository to either build it localy or install it as a shared library.
If build localy these files needs to be linked when building the server. See non-enclave-server/Makefile regarding that.

run ./configure
The following modules included:
gsl/histogram/.libs
gsl/err/.libs
linalg/.libs
fit/.libs
...
or `make run` (will default to use mbedtls)
Run `make run-server-in-loop`
